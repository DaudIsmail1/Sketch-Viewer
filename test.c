...
static void testBasicOpcodes(void) {
...
static void testBasicOperands(void) {
...
static void testBasicCommand(void) {
...
display *newDisplay(char *name, int width, int height) {
  ...
  *d = (struct display) { name, findTest(name), width, height, 0, 0};
  return d;
}

void run(display *d, void *data, bool action(display *, const char, void*)) {
  int targets[] = {153, 612, 306, 357, 510, 204, 408, 663, 510, 255};
  static int pos = 0;
  bool quit = false;
  char key = 0;
  char rounds = 0;
  while (!quit) {
    if (rounds > 50) key = 27;
    quit = action(d, key, data);
    quit = quit && (key == 27);
    sprintf(d->call, "processSketchReturn");
    check(d);
    rounds++;
  }
  if ((d->all) != (targets[pos])) {
    fprintf(stderr, "ERROR: When processing %s your processSketch procedure does not produce the correct full sequence of drawing interactions, most likely because it ends too early or does not return or exit correctly as required by the task.\n", getName(d));
    exit(1);
  }
  pos++;
}
